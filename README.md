...
This project goes beyond standard DEG analysis by incorporating **Gene Ontology (GO) enrichment analysis** — a powerful computational approach that asks not just *which genes* are differentially expressed, but *what biological processes* those genes are collectively involved in.
...
| **GO Enrichment** | clusterProfiler — Biological Process ontology |
...
2. Download the GSE4302 dataset from [NCBI GEO](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE4302) and place it in the `data/` folder
3. Open `scripts/asthma_analysis.R` in RStudio
...
Key biological questions explored:
- Which inflammatory pathways are suppressed by Flovent treatment?
- Are immune response genes significantly downregulated in treated patients?
